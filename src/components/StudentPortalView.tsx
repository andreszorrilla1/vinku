...
function PassportTab({ student }: { student: Student }) {
  ...
  const hasStamps = student.passport.sellos.length > 0;
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="px-6 py-5">
          ...
          {!hasStamps ? (
            <div className="text-center py-10">
              <div className="w-20 h-20 rounded-full border-4 border-dashed border-zinc-700 flex items-center justify-center mx-auto mb-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#FFD000] rounded-xl p-4 text-center">
          <p className="text-3xl font-display font-extrabold text-[#1A1A1A]">{student.passport.sellos.filter(s => s.status === "Certificado").length}</p>
...
function DiagnosticTab({
  ...
  function selectOption(key: string, value: string) {
    ...
    setDiagAnswers((prev: any) => ({ ...prev, [key]: value }));
  }
  ...
  if (step === 4) {
    const suggested = courses.filter(c =>
      student.suggestedRoute.includes(c.id) ||
      c.category === (diagAnswers.primaryGoal === "Engineering" ? "Tecnología" : diagAnswers.primaryGoal === "Marketing" ? "Marketing" : "Datos")
          ...
          {suggested.length === 0 ? (
            ...
            suggested.map((c, i) => (
              <div key={c.id} className="bg-white border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#6C47FF] rounded-xl p-4 flex items-center gap-4">
                ...
                <span className="text-xs font-bold text-[#6C47FF] shrink-0">{formatCOP(c.cost)}</span>
              </div>
            ))
          )}
        </div>
      ...
      {studentTab === "wallet" && (
        <div className="space-y-5 animate-fade-in max-w-md">
          ...
          <div className="bg-white border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#6C47FF] rounded-xl p-5">
            ...
            <form onSubmit={onWalletRecharge} className="space-y-3">
              <input
                type="number"
                value={rechargeAmt}
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {MENTORS.map(mentor => (
              <div
                key={mentor.name}
                onClick={() => setFellowshipForm((prev: any) => ({ ...prev, mentorName: mentor.name }))}
          ...
          <form onSubmit={onBookFellowship} className="bg-white border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#6C47FF] rounded-xl p-5 space-y-4 max-w-md">
            ...
            <input
              ...
              onChange={e => setFellowshipForm((p: any) => ({ ...p, dateTime: e.target.value }))}
        ...
        <div className="space-y-5 animate-fade-in">
          <div className="space-y-3">
            {student.passport.logros.length === 0 ? (
              ...
              student.passport.logros.map((logro, i) => (
                <div key={i} className="bg-white border-2 border-[#1A1A1A] shadow-[3px_3px_0px_0px_#1A1A1A] rounded-xl p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-full border shrink-0 ${
                      logro.status === "Cumplido" ? "bg-[#10B981]/10 text-[#10B981] border-[#10B981]/30"
                        : logro.status === "En Progreso" ? "bg-amber-400/10 text-amber-600 border-amber-400/30"
          ...
          <form onSubmit={onAddGoal} className="bg-white border-2 border-[#1A1A1A] shadow-[4px_4px_0px_0px_#FFD000] rounded-xl p-5 space-y-3 max-w-md">
